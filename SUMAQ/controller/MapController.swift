...
//  SUMAQ
//
//  MULTITRHEADING - Strategy #3 Swift Concurrency (async/await) : Maria
//
//
...
final class MapController: ObservableObject {
    ...
    func loadRestaurants() async {
        ...
        do {
            ...
            // Prepare a small, bounded parallelism
            let maxConcurrent = 4
            var index = 0
            ...
            // Use a throwing task group so any thrown error cancels siblings
            ...
            //  annotations and publish on main thread
            await MainActor.run {
            ...
            await MainActor.run {
                // When remote fetch fails offline, also clear pins and center so the UI
                // can detect the absence of valid map data and show the offline message.
                self.errorMsg = error.localizedDescription //  Preserve previous behavior
                self.annotations = []                      //  avoid showing old pins.
                self.center = nil                          //   HomeUserView won't render the map.
            }
        }
    }

    // MARK: - Per-restaurant coordinate resolution
    ...
    private static func resolveCoordinate(for r: Restaurant) async throws -> (Restaurant, CLLocationCoordinate2D?) {
        ...
        // geocode with a fresh geocoder instance for safe to run in parallel
    ...
    /// instance per request allows safe parallel geocoding across tasks
